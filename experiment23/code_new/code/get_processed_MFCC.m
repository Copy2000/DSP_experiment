clear all;
clc;
WinType = "hanning";  %窗口类型 rectangle(boxcar) hanning hamming
%% number
% for i = 1:10
%     for k=0:9
%         for j = 1:2
...
%             data=load(filename);
%             m=MFCC(data,WinType);
%             processed_data_filename="../data/MFCC_feature/MFCC_processed_num/"+"MFCC_Processed_"+WinType+filename0+".txt"
...
for i = 1:5
    for k=1:5
        for j = 1:4
            ...
            data=load(filename);
            m=MFCC(data,WinType);
            processed_data_filename="../data/MFCC_feature/MFCC_processed_name/"+"MFCC_Processed_"+WinType+filename0+".txt"
            save(processed_data_filename,'m','-ascii');
        end
    end
end