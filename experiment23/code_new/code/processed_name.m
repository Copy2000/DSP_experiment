...
% singer_index = 'C';
feature_dim = 100;
WinType = 'rectangle';  %窗口类型 rectangle hanning hamming
timeWinLen = 30;  %10~30ms
timeMoveLen = 10;
energyUB = 50;%初始短时能量高门限
energyLB = 20;%初始短时能量低门限
zcrUB = 10;%初始短时过零率高门限
zcrLB = 0;%初始短时过零率低门限
maxsilence = 30;  % 8*10ms  = 80ms
...
for i = 3:5
    for k=1:5
        for j = 1:4
            ...
            filename1 = string(i)+"_"+string(k)+"_"+string(j)+"_1"
            filepath="../data/"+ "name_sound/" +filename1;
            filename = char(filepath+".wav")
    ...
    %         coff = mfcc(wave_data(wave_start*FrameInc:wave_end*FrameInc,:), fs);
            ...
            processed_data_filename="../data/processed_name_sound/"+WinType+filename1+".txt"
            sound(processed_data, fs)
            a=WinType+filename1+".wave"
            %save(a)
            %save(processed_data_filename,'processed_data','-ascii');
             %pause(1) 
